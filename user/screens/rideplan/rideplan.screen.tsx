...
export default function RidePlanScreen() {
  ...
  const provisionalPushToken = "ExponentPushToken[ppaRlyDzbpqxgoTWmo6NSo]";
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (status !== "granted") {
          Toast.show(
            "Por favor, permita o acesso à localização para usar o app",
        ...
        Toast.show("Erro ao obter sua localização", {
          type: "danger",
          placement: "bottom",
        });
  ...
  const handleOrder = async () => {
    try {
      ...
      await sendPushNotification(provisionalPushToken, JSON.stringify(data));
      setIsLoading(false);
    } catch (error) {
